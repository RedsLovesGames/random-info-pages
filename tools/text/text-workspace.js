...
function commitInput(label) {
  history.push({ label, state: textInput.value });
  renderHistory();
  syncHistoryButtons();
}
...
$('#useOutput').addEventListener('click', () => queueMicrotask(() => commitInput('Use output as input')));
$('#swapText').addEventListener('click', () => queueMicrotask(() => commitInput('Swap input/output')));
