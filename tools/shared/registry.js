...
export const TOOLBOX_REGISTRY = [
  ...
  },
  {
    ...
    type: 'input', privacy: 'Local', available: true, keywords: ['math', 'calculator', 'physics', 'chemistry', 'units', 'algebra', 'geometry', 'grades'],
    actions: [
      action('calculator', 'Calculator', 'math', ['scientific calculator', 'expression calculator']),
      action('units', 'Unit Converter', 'math', ['convert units', 'unit conversion']),
      action('algebra', 'Algebra', 'math', ['quadratic equation', 'quadratic formula', '2x2 system', 'matrix determinant']),
      action('graph', 'Graph', 'math', ['graph equation', 'function plot', 'plot function']),
      action('geometry', 'Geometry', 'math', ['circle area', 'sphere volume', 'triangle area', 'cylinder volume']),
      action('physics', 'Physics', 'science', ['kinematics', 'vectors', 'kinetic energy', 'potential energy']),
      action('chemistry', 'Chemistry', 'science', ['molar mass', 'dilution', 'moles', 'chemical formula']),
      action('electronics', 'Electronics', 'science', ['ohms law', 'voltage divider', 'resistor', 'parallel resistance']),
      action('grades', 'Grade Calculator', 'school', ['weighted grade', 'final grade', 'required final grade']),
    ],
  },
  {
    ...
    type: 'input', privacy: 'Local', available: true, keywords: ['random', 'picker', 'weighted random picker', 'teams', 'dice', 'shuffle', 'sample', 'seed'],
    actions: [
      action('pick', 'Random Pick', 'random', ['random picker', 'weighted random picker', 'choose one']),
      action('shuffle', 'Shuffle', 'random', ['shuffle list', 'random order']),
      action('teams', 'Make Teams', 'random', ['random teams', 'groups']),
      action('numbers', 'Random Numbers', 'random', ['number generator', 'random integers']),
      action('chance', 'Chance', 'random', ['coin flip', 'dice', 'percentage chance', 'probability trials']),
      action('seed', 'Seeded Random', 'random', ['seed rng', 'reproducible random', 'seeded random']),
    ],
  },
  {
    id: 'codes', title: 'Codes & Generator Lab', route: './codes/', description: 'Generate and scan QR/barcodes, passwords, tokens, and IDs.',
    type: 'input', privacy: 'Local', available: true, keywords: ['qr', 'barcode', 'data matrix', 'password', 'uuid', 'secure token'],
    actions: [
      action('qr', 'QR Code', 'codes', ['qr generator', 'wifi qr', 'vcard qr', 'email qr']),
      action('barcode', 'Barcode', 'codes', ['barcode generator', 'code 128', 'ean', 'upc', 'data matrix barcode']),
      action('scan', 'Scan Code', 'codes', ['qr scanner', 'barcode scanner', 'scan code']),
      action('password', 'Password', 'generate', ['password generator', 'passphrase', 'secure password']),
      action('uuid', 'UUID / Token', 'generate', ['uuid generator', 'guid', 'secure token', 'random token']),
    ],
  },
  {
    id: 'network', title: 'Network & System Lab', route: './network/', description: 'Subnet, storage, transfer, display, and device calculations.',
    type: 'input', privacy: 'Local', available: true, keywords: ['network', 'subnet', 'cidr', 'ipv4', 'ipv6', 'bandwidth', 'storage', 'ppi', 'browser capabilities'],
    actions: [
      action('subnet', 'IP / Subnet', 'network', ['cidr', 'subnet calculator', 'ipv4', 'ipv6', 'ipv6 cidr']),
      action('storage', 'Storage Units', 'data', ['kb mb gb tb', 'bits bytes', 'gib mib']),
      action('transfer', 'Transfer Time', 'data', ['download time', 'upload time', 'bandwidth calculator']),
      action('media-size', 'Media Size', 'data', ['bitrate file size', 'media storage']),
      action('display', 'Display', 'system', ['aspect ratio', 'ppi', 'ppi calculator', 'resolution']),
      action('device', 'Device Capabilities', 'system', ['browser info', 'browser capabilities', 'webgpu', 'codecs']),
    ],
  },
];
