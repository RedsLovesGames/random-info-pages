(function(root,factory){const G=typeof module==='object'&&module.exports?require('./map-geometry.js'):root.MapGeometry;const api=factory(G);if(typeof module==='object'&&module.exports)module.exports=api;else root.RecommendedPlays=api;})(typeof self!=='undefined'?self:this,function(G){
    ...
    return{
      attack:[
        attack(map,'mid-control','Mid control default','Slow',[
          slot('entry','Mid pressure','mid','bLink',['primaryEntry','secondaryEntry'],{postPlantAnchor:'postB'}),
          slot('second','A pressure','midLeft','aLink',['secondaryEntry','flashEntry','lurker'],{postPlantAnchor:'midLeft'}),
          slot('info','B Main info','bMain','siteB',['infoInitiator','execInitiator'],{postPlantAnchor:'postB'}),
          slot('global','Central support','flankLeft','aMain',['globalController','localController'],{postPlantAnchor:'midLeft'}),
          slot('lurk','B extremity','flankRight','bMain',['lurker','flankControl','localController'],{postPlantAnchor:'flankRight'})
        ...
        attack(map,'a-split','A split through mid','Medium',[
          slot('entry','A Main entry','aMain','siteA',['primaryEntry'],{postPlantAnchor:'postA'}),
          slot('second','Mid split','midLeft','siteA',['secondaryEntry','flashEntry'],{postPlantAnchor:'aLink'}),
          slot('info','A support','flankLeft','aMain',['infoInitiator','execInitiator'],{postPlantAnchor:'aMain'}),
          slot('global','Mid controller','mid','aLink',['globalController','localController'],{postPlantAnchor:'midLeft'}),
          slot('lurk','B hold','bMain','midRight',['lurker','flankControl','localController'],{postPlantAnchor:'midRight'})
  ...
  DB.Haven={
    attack:[
      attack('Haven','three-lane-default','Three-lane default','Slow / reactive',[
        slot('entry','A pressure','aMain','siteA',['primaryEntry','secondaryEntry'],{postPlantAnchor:'aLink'}),
        slot('second','Garage contact','garage','siteB',['secondaryEntry','flashEntry','execInitiator'],{postPlantAnchor:'bLink'}),
        slot('info','C Long information','cLong','siteC',['infoInitiator','execInitiator'],{postPlantAnchor:'postC'}),
        slot('global','Mid support','mid','aLink',['globalController','localController'],{postPlantAnchor:'cLink'}),
        slot('lurk','C extremity','flankRight','cLong',['flankControl','lurker','localController'],{postPlantAnchor:'cLong'})
      ...
      attack('Haven','a-explode','A explode','Fast',[
        slot('entry','A Long entry','aMain','siteA',['primaryEntry'],{postPlantAnchor:'postA'}),
        slot('second','A Short trade','midLeft','siteA',['secondaryEntry','flashEntry'],{postPlantAnchor:'aLink'}),
        slot('info','A utility support','flankLeft','aMain',['infoInitiator','execInitiator'],{postPlantAnchor:'aMain'}),
        slot('global','Mid smoke support','mid','aLink',['globalController','localController'],{postPlantAnchor:'midLeft'}),
        slot('lurk','C hold','cLong','midRight',['flankControl','lurker','localController'],{postPlantAnchor:'cLong'})
  ...
  function validate(play){
    ...
    for(const s of play.slots){
      if(!G.anchor(play.map,s.startAnchor))throw new Error(`missing anchor ${s.startAnchor}`);
      if(s.destinationAnchor&&!G.path(play.map,s.startAnchor,s.destinationAnchor).length)throw new Error(`invalid route ${s.id}`);
      if(play.side==='attack'){
        if(!s.postPlantAnchor||!G.anchor(play.map,s.postPlantAnchor))throw new Error(`attack slot missing post-plant anchor ${s.id}`);
        if(s.destinationAnchor&&!G.path(play.map,s.destinationAnchor,s.postPlantAnchor).length)throw new Error(`invalid post-plant route ${s.id}`);
      }
    }
